...
class RODnet(nn.Module):
    def __init__(self,in_channels,n_class):
        ...
        self.finalConv_velo = nn.Sequential(
            nn.Conv2d(in_channels=32,out_channels=16,stride=(2,2),padding=(2,2),kernel_size=(5,5)),
            nn.PReLU(),
            nn.Conv2d(in_channels=16,out_channels=1,stride=(2,2),padding=(2,2),kernel_size=(5,5)),
            nn.Tanh()
        )
    ...
    def forward(self,ra,rd,ad):
        ...
        x_velo = self.finalConv_velo(x) # (B,32,256,256) -> (B,2,64,64)
        #ra,rd_1,ad_1,ra_attention,rd_b,ad_b
        

        return x_cls,x_center,x_orent,x_velo



def test():
    ra= torch.randn(5,5,256,256)
    rd= torch.randn(5,5,256,64)
    ad= torch.randn(5,5,64,256)
    model = RODnet(in_channels=5,n_class=3)
    out_cls,out_center,out_orent,out_velo = model(ra,rd,ad)
    print(out_cls.shape,out_center.shape,out_orent.shape,out_velo.shape)
