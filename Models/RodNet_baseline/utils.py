...
def check_accuracy(loader, model,step,writer,focal_loss,device="cuda"):
    ...
    with torch.no_grad():
        mse_loss =[]
        f_loss = []
        for ra,rd,ad,tr_map,_ in loader:
    ...
    print(f"mean MSE : {mMSE}",f"mean focal : {mfocal}")
...
def save_predictions_as_numpy(
    ...
    for idx, (ra,rd,ad,tr_map,name) in tqdm(enumerate(loader)):
...
def evaluation(
    ...
    with torch.no_grad():
        ...
        for idx, (ra,rd,ad,tr_map,name) in enumerate(tqdm(loader)):
            if idx>0 and idx<64:
                ra = ra.to(device)
                rd = rd.to(device)
                ad = ad.to(device)
            ...
            #cls = metrics(y,preds,cls, thresh=3)
                cls = metrics_center(grd_map,pred_map,mask,peak_cls,cls,thresh=2,device=device)
